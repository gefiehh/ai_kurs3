...
from voice import listen  # Импорт голосового ввода
...
class ChatBot:
    def __init__(self):
        ...
        # Инициализация голосового сервиса (TTS)
    ...
    def run(self):
        print("Введите 'г' для голосового ввода, 'т' для текстового")
        
        while True:
            mode = input("\nВыберите режим (г/т): ").strip().lower()
            
            if mode in ["выход", "quit", "exit"]:
                response = "До свидания!"
                print(f"Бот: {response}")
                self.voice.speak(response)
                break
            
            if mode in ["г", "voice", "v"]:
                time.sleep(0.3)  # Небольшая пауза перед записью
                user_input = listen()
                if not user_input:
                    print("Не удалось распознать речь")
                    continue
                print(f"Вы: {user_input}")
            elif mode in ["т", "text", "t"]:
                user_input = input("Вы: ").strip()
            else:
                print("Неверный выбор. Введите 'г' или 'т'")
                continue
